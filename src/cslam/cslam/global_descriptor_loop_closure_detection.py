...
class GlobalDescriptorLoopClosureDetection(object):
    ...
    def __init__(self, params, node):
        ...
        if self.params['frontend.global_descriptor_technique'].lower(
            ...
            from cslam.vpr.anyloc import AnyLocVPR
            self.node.get_logger().info('Using AnyLoc (DINOv2 via ZMQ server).')
            self.global_descriptor = AnyLocVPR(self.params, self.node)
            self.keyframe_type = "rgb"
        elif self.params['frontend.global_descriptor_technique'].lower(
        ) == 'cosplace_server':
            from cslam.vpr.cosplace_server_client import CosPlaceServerClient
            self.node.get_logger().info('Using CosPlace (remote server mode via ZMQ).')
            self.global_descriptor = CosPlaceServerClient(self.params, self.node)
            self.keyframe_type = "rgb"
        else:
