...
{
  if (local_descriptors_map_.count(request->keyframe_id) == 0)
  {
    RCLCPP_WARN(node_->get_logger(),
                "local_descriptors_request: keyframe %d not in map, skipping",
                request->keyframe_id);
    return;
  }
  // Fill msg
...
void RGBDHandler::receive_local_keyframe_match(
    cslam_common_interfaces::msg::LocalKeyframeMatch::ConstSharedPtr
        msg)
{
  // Check keyframes are still in the local window before attempting registration
  if (local_descriptors_map_.count(msg->keyframe0_id) == 0 ||
      local_descriptors_map_.count(msg->keyframe1_id) == 0)
  {
    return;  // keyframe evicted from sliding window — skip silently
  }
  try
  {
